...
namespace Serilog.Tests.Enrichers
{
    [TestFixture]
    [Parallelizable]
    public class PathEnricherTests
    {
        [SetUp]
        public void SetUp()
        {
            ...
            _logger = new LoggerConfiguration()
                .Enrich.With(new PathEnricher(_httpContextProvider.Object))
        ...
        public void ShouldCreatePathProperty()
        {
            ...
            _httpRequestWrapper.SetupGet(x => x.Path).Returns(expected);

            _logger.Information(@"Has a Path property");

            Assert.NotNull(_logEvent);
            Assert.NotNull(_logEvent.Properties["Path"].LiteralValue());
            Assert.AreEqual($"\"{expected}\"", _logEvent.Properties["Path"].LiteralValue());
        }
    }
}